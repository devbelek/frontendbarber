...
const HomePage = ({ openLoginModal }) => {
  ...
  const handleFavoriteToggle = async (haircutId, e) => {
    ...
    try {
      ...
      setPopularHaircuts((prev: any) =>
        prev.map((h) =>
          h.id === haircutId ? { ...h, is_favorite: !h.is_favorite } : h
        )
      );
      setAllHaircuts((prev: any) =>
        prev.map((h) =>
          h.id === haircutId ? { ...h, is_favorite: !h.is_favorite } : h
        )
      );
  ...
  const handleBookClick = (haircut: any) => {
    setSelectedHaircut(haircut);
  ...
  const HaircutCard = ({ haircut }) => {
    ...
    return (
      <div className="bg-white rounded-lg overflow-hidden shadow-md border  hover:shadow-xl transition-all duration-300">
        <div className="relative aspect-square">
          ...
          {hasMultipleImages && (
            <>
              <button
                ...
                className="absolute left-2 top-1/2 -translate-y-1/2 p-2 bg-black/60 text-white rounded-full hover:bg-black/80 transition-all sm:p-3"
              >
                <ChevronLeft className="h-4 w-4 sm:h-5 sm:w-5" />
              </button>
              <button
                onClick={handleNextImage}
                className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-black/60 text-white rounded-full hover:bg-black/80 transition-all sm:p-3"
              >
          ...
          <div className="absolute top-2 right-2 flex gap-1 z-10">
            ...
            {(haircut.barberWhatsapp || haircut.barberTelegram) && (
              <button
                className="p-1.5 rounded-full bg-black/30 backdrop-blur-sm hover:bg-black/50 transition-colors text-white sm:p-2"
        ...
        <div className="p-3 sm:p-4">
          ...
          <div className="flex justify-between items-center mb-2">
            <span className="text-[#9A0F34] font-bold text-sm sm:text-base">
              {Math.floor(haircut.price || 0)} сом
            </span>
            <button
              onClick={(e) => handleBarberClick(haircut.barberId, e)}
              className="text-xs sm:text-sm text-gray-600 hover:text-[#9A0F34] transition-colors"
            >
              {haircut.barber}
            </button>
          </div>
          <button
            className="w-full bg-[#9A0F34] text-white text-sm sm:text-base py-2 rounded-lg hover:bg-[#7b0c29] transition-colors"
            onClick={async () => {
              try {
  ...
  return (
    <Layout openLoginModal={openLoginModal}>
      <div className="pt-16 font-['Inter'] max-w-7xl mx-auto">
        <Banner />

        {/* Поисковая панель */}
        <div className="sticky top-16 z-20 bg-white shadow-md py-3 sm:py-4">
          {userLocation.address && (
            <div className="flex items-center justify-center mb-2 text-sm sm:text-base text-gray-600">
              <MapPin className="h-4 w-4 sm:h-5 sm:w-5 mr-1 text-[#9A0F34]" />
          ...
          <div className="hidden md:flex items-center justify-center gap-3 sm:gap-4 px-4">
            <div className="relative w-full max-w-xs" ref={searchInputRef}>
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" />
              <input
                type="text"
                placeholder="Найти стрижку..."
                className="w-full pl-10 pr-10 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#9A0F34] focus:outline-none transition-shadow text-sm sm:text-base"
            ...
            <div className="relative">
              <button
                className={`flex items-center h-full px-3 py-2 border border-gray-300 rounded-lg ${
              ...
              {showCategoryDropdown && (
                <div className="absolute top-full right-0 mt-2 bg-white rounded-lg shadow-xl border border-gray-100 p-3 z-40 w-64 sm:w-72">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {categories.map((category) => (
                      <button
                        ...
                        className={`flex flex-col items-center p-2 rounded-lg hover:bg-gray-50 transition-colors ${category.color} text-xs sm:text-sm`}
                      >
                        <Scissors className="h-4 w-4 sm:h-5 sm:w-5 mb-1" />
            ...
            <div className="flex gap-2">
              <div className="relative flex-grow">
                ...
                <input
                  ...
                  onChange={(e) => setSearchQuery(e.target.value)}
                  onKeyDown={handleSearchKeyDown}
                  className="w-full pl-10 pr-10 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#9A0F34] focus:outline-none text-sm"
                />
                {searchQuery && (
                  <button
        ...
        <div className="py-4 px-4 sm:px-6 bg-gray-50">
          <div className="flex justify-between items-center mb-3">
            <h2 className="text-lg sm:text-xl font-semibold">Барберы рядом</h2>
            <button
              onClick={() => navigate("/discover")}
              className="text-sm sm:text-base text-[#9A0F34] hover:underline"
            >
              Смотреть все
            </button>
          </div>
              ...
              {loading ? (
                Array(4)
                  ...
                  .map((_, i) => (
                    <div
                      key={i}
                      className="flex-shrink-0 w-32 sm:w-36 bg-white rounded-lg p-3 shadow-sm animate-pulse"
                    >
                      <div className="w-14 h-14 sm:w-16 sm:h-16 bg-gray-200 rounded-full mx-auto mb-3"></div>
                ...
                nearbyBarbers.map((barber) => (
                  <button
                    ...
                    className="flex-shrink-0 w-32 sm:w-36 bg-white rounded-lg p-3 shadow-md hover:shadow-xl transition-all duration-300"
                  >
                    <img
                      src={
                      ...
                      className="w-14 h-14 sm:w-16 sm:h-16 rounded-full mx-auto mb-2 object-cover"
                      loading="lazy"
                    />
        ...
        <div className="py-4 px-4 sm:px-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
            {loading ? (
              ...
              <div className="col-span-2 sm:col-span-3 lg:col-span-4 text-center py-4 text-gray-500 text-sm sm:text-base">
                Стрижки не найдены
              </div>
            )}
          </div>
        ...
        <div className="py-4 px-4 sm:px-6 bg-gray-50">
          ...
          <div className="flex overflow-x-auto -mx-4 px-4 space-x-3 pb-2">
            {[
              {
                icon: (
                  <Search className="h-6 w-6 sm:h-8 sm:w-8 text-[#9A0F34]" />
                ),
              ...
              {
                icon: (
                  <Calendar className="h-6 w-6 sm:h-8 sm:w-8 text-[#9A0F34]" />
                ),
                title: "Забронируй время",
                desc: "Запишись к барберу онлайн",
              },
              {
                icon: <Star className="h-6 w-6 sm:h-8 sm:w-8 text-[#9A0F34]" />,
              ...
              <div
                key={index}
                className="flex-shrink-0 w-36 sm:w-44 p-3 bg-white rounded-lg shadow-sm"
              >
        ...
        {/* Все стрижки */}
        <div className="py-6 px-4 sm:px-6" data-section="gallery">
          <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-3">
            <h2 className="text-xl sm:text-2xl font-semibold">Все стрижки</h2>
          ...
          {!loading && !galleryLoading && allHaircuts.length === 0 && (
            <div className="text-center py-12">
              ...
              <Button className="text-sm sm:text-base" onClick={resetFilters}>
                Сбросить фильтры
              </Button>
            </div>
          )}
          ...
          <div className="fixed inset-0 z-50 bg-black/50 md:hidden">
            <div className="absolute inset-x-0 bottom-0 bg-white rounded-t-xl p-4 max-h-[80vh] overflow-y-auto">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg sm:text-xl font-semibold">Фильтры</h3>
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {["popular", "price", "recent"].map((option) => (
                    <button
                      key={option}
              ...
              <div className="flex gap-2 mt-6">
                <button
                  className="w-full py-2 bg-gray-200 rounded-lg text-sm sm:text-base"
                  onClick={resetFilters}
                >
                  Сбросить
                </button>
                <button
                  className="w-full py-2 bg-[#9A0F34] text-white rounded-lg text-sm sm:text-base"
                  onClick={() => {
                    setIsFilterOpen(false);
                    handleSearch();
                  }}
        ...
        {showContactModal && selectedHaircut && (
          ...
          >
            ...
            >
              ...
              <div className="space-y-3">
                {selectedHaircut.barberWhatsapp && (
                  <a
                    ...
                    className="flex items-center justify-center w-full bg-gradient-to-r from-green-500 to-green-600 text-white py-3 sm:py-4 rounded-2xl hover:shadow-lg transition-all duration-300 font-medium text-sm sm:text-base"
                  >
                    <svg
                ...
                {selectedHaircut.barberTelegram && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center justify-center w-full bg-gradient-to-r from-blue-500 to-blue-600 text-white py-3 sm:py-4 rounded-2xl hover:shadow-lg transition-all duration-300 font-medium text-sm sm:text-base"
                  >
                    <svg
                      className="w-5 h-5 sm:w-6 sm:h-6 mr-2 sm:mr-3"
                      fill="currentColor"
              ...
              <button
                onClick={() => setShowContactModal(false)}
                className="mt-6 w-full text-gray-500 py-3 hover:text-gray-700 transition-colors font-medium text-sm sm:text-base"
              >
                Закрыть
              </button>
            </div>
