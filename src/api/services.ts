...
export const notificationsAPI = {
  registerTelegramAccount: (data: any) =>
    apiClient.post("/notifications/register-telegram/", data),
...
export const bookingsAPI = {
  ...
  cancel: (id: string) =>
    apiClient.patch(`/bookings/${id}/`, { status: "cancelled" }),
...
export const servicesAPI = {
  ...
  getPopular: () =>
    apiClient
      .get("/services/", { params: { limit: 6, ordering: "-views" } })
      .catch((error: any) => {
        console.log(
          "Using demo data for popular services due to error:",
          error
        );
        return { data: demoHaircuts };
      }),
  ...
  create: (data: any) => {
    ...
    if (data instanceof FormData) {
      ...
      console.log(
        "Creating service with FormData:",
  ...
  update: (id: string, data: any) => {
    ...
    if (data instanceof FormData) {
      // Логируем для отладки
      console.log(
        `Updating service ${id} with FormData:`,
        Array.from(data.entries())
      );
...
export const locationAPI = {
  getNearbyBarbers: (latitude: number, longitude: number, radius: number = 5) =>
    apiClient.get(
      `/services/?latitude=${latitude}&longitude=${longitude}&radius=${radius}`
    ),
  getRecommendations: (latitude: number, longitude: number) =>
    apiClient.get(
      `/services/recommendations/?latitude=${latitude}&longitude=${longitude}`
    ),
};

export const authAPI = {
  login: (credentials: any) =>
    axios.post(`${API_URL}/auth/jwt/create/`, credentials),
  register: (userData: any) => axios.post(`${API_URL}/auth/users/`, userData),
  getCurrentUser: () => apiClient.get("/auth/users/me/"),
  resetPassword: (email: string) =>
    axios.post(`${API_URL}/auth/users/reset_password/`, { email }),
  refreshToken: (refresh: string) =>
    axios.post(`${API_URL}/auth/jwt/refresh/`, { refresh }),
